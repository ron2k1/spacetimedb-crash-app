// marketplace-server/src/runtime/search.ts
//
// Tavily-backed search. Lifted from backend/src/connectors/search.ts (imports made local).
// `fetchImpl` is injected for testability. On failure, if `canned` results are supplied the call
// degrades gracefully (flop-proof) and still reports 'ok'; with no canned fallback it reports
// 'error' with a synthetic code. The response body is consumed for hits only, never logged.
