// marketplace-server/src/runtime/buyer.ts
//
// The x402 buyer: a fetch WRAPPER that pays an HTTP 402 challenge and retries. Lifted from
// backend/src/payments/buyer.ts (imports made local). x402 is modeled as orthogonal to the
// wrapped request's own auth, so any connector (e.g. the Tavily search POST) can be paid for
// without changing its descriptor.
//
