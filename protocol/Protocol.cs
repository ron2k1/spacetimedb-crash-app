...
namespace Crash.Protocol
{
    ...
    [Serializable] public class TerminalOutputPayload { public string requestId; public string stream; public string line; public int seq; } // stream: 'stdout'|'stderr'; line: raw CLI output for the read-only Technical mirror -- RENDERER-ONLY, never logged/persisted
    [Serializable] public class ProviderSessionPayload { public string requestId; public string provider; public string state; public string detail; } // provider worker lifecycle for Technical; never includes the provider session id
    ...
    // ---- v3 additions ----
    [Serializable] public class ProviderAuth { public string id; public bool installed; public bool authenticated; } // id: 'claude-code' | 'codex'; booleans derived from exit codes / file EXISTENCE, never contents
    [Serializable] public class AuthStatusQueryPayload {} // Renderer -> Engine: request a fresh auth snapshot
    [Serializable] public class AuthLoginStartPayload { public string provider; } // provider: 'claude-code' | 'codex'
    [Serializable] public class AuthStatusPayload { public ProviderAuth[] providers; public string active; } // active: live provider id, or null
    [Serializable] public class AuthLoginResultPayload { public string provider; public bool launched; public string code; } // code: SYNTHETIC failure code only, never a message
