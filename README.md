...
live auction's bids, the clock ticking down, the winning order -- all of it is just rows changing
...
```
            Today                                  Target (this hackathon)
  +-----------------------------+        +-------------------------------------+
  |  marketplace-server :8787   |        |          SpacetimeDB module         |
  |  Express + ws               |        |  tables: listing / auction / bid /  |
  |  in-memory MarketStore      |  --->  |          order / agent / activity   |
  |  EventEmitter fan-out       |        |  reducers: create_listing /         |
  |  data/listings.json persist |        |    place_bid / buy_now /            |
  |  manual ws broadcast        |        |    create_auction / settle_auction  |
  +-----------------------------+        |    (scheduled) / register_agent /   |
                                         |    record_payment                   |
...
| `order` | A settled sale: buyer, listing, price, and a payment status (`awaiting_payment` / `settled`). |
...
| `buy_now` | Create a settled-pending order; mark the listing; append activity. |
| `create_auction` | Open an auction with an end time. |
| `place_bid` | Validate against the current high bid, record the bid, raise the auction. |
| `settle_auction` | **Scheduled** -- fires at `ends_at`, picks the winner, writes the order `awaiting_payment`. |
| `register_agent` | Enroll an agent participant under its identity. |
| `record_payment` | Write a settlement result (txRef / failure code) back onto an order. |
...
- **Payment is a side-channel, by necessity.** Reducers are deterministic and sandboxed -- no
  outbound HTTP or chain calls inside a reducer. So `settle_auction` marks the order
...
|-------------|--------------------|
| **SpacetimeDB is the primary backend** | The module's tables + reducers are the marketplace's entire state and write API; the storefront server is being deleted, not wrapped. |
...
|-------|--------------------|
| **Heavily real-time** | Live auctions: bids, the countdown, and the winning order are table deltas streamed to every client as they commit. |
...
| Storefront (today) | `@crash/marketplace-server` -- Express + `ws`, the in-memory source of truth being ported to SpacetimeDB |
