// PlayingCard -- a 3D-shader "trading card" with an optional dynamic reveal-canvas background,
// used as the visual for the skills a user creates. Adapted from a Next.js registry component
// for this Vite + Tauri + React 19 app:
//   * dropped the "use client" directive (Next-only; no-op in Vite)
//   * replaced next/image's <Image fill> with a plain <img> (so we do NOT pull in `next`); the
//     image already lives in a fixed-aspect box with an explicit height, so width/height:100%
//     + object-fit:contain reproduces `fill` exactly
//   * import cn from the project's @/lib/utils instead of an inline copy
//   * widened the useDynamicTextLayout containerRef param to RefObject<HTMLDivElement | null>,
//     because React 19's useRef<T>(null) now returns RefObject<T | null>
// three + @react-three/fiber are already project deps (see dotted-surface / spotlight / splite).
